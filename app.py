...
from src.models import InvoiceResult, InvoiceResultMetadata
from src.logic import logic_rules
from src.utils import load_document, gather_with_concurrency, create_hash

# todo also add logs for the case that the user does not use Logfire.
load_dotenv()
...
async def extract_invoice_batch(agent, files):
    tasks = []
    for file in files:
            # Prepare each image
            ...
            # Add to task list
            tasks.append(agent.extract_data(img))
    ...
    if st.button(f"Process {len(uploaded_files)} Invoices"):
        ...
        with st.spinner(f"Processing {len(uploaded_files)} files... This may take a moment."):
            ...
            for uploaded_file, run_result in zip(uploaded_files, results):
                ...
                # Check if the hash value is already seen else
                if hash_val not in st.session_state.processed_hashes:
                    ...
                    status = 'REJECT'
                    reasons = ["Duplicate invoice detected based on hash. This invoice has already been processed."]
    ...
    for i, res in enumerate(st.session_state.current_batch):
        ...
        with st.expander(f"{res.metadata.filename}: Status: {':red[REJECTED]' if status == 'REJECT' else ':green[ACCEPTED]'}"):
            ...
            st.download_button(
                ...
                file_name=f"result_{res.metadata.filename}_{res.metadata.created_time.strftime('%Y%m%d_%H%M%S')}.json",
...
def session_history_log():
    if st.session_state.history:
        ...
        history_data = [
            {
                ...
                "Total Amount Gross": f"{h.extracted_data.total_amount_gross:.2f}",
        ...
        st.download_button(
            ...
            data=full_json,
            file_name=f"invoice_results_history_{datetime.datetime.now()}.json",
            mime="application/json"
        )
